...
logger = make_logger('TEST_file-process', log_file='logs/TEST_file_process.log')
...
config.read("TEST_file_processing 0621.config", encoding="utf-8-sig")
...
def send_to_score(conn_sql, cur, pre_list, dir, num, ip, logger, score_socket):
    ...
    zip_name = str(num)+".7z"
    folder_path = os.path.join(target_dir, data_list[0], data_list[1], data_list[2], data_list[3])
...
def datalist_inser_to_sql(conn_sql, cur, datalist, dir_file_path, ip, num):
    ...
    cur.execute("INSERT INTO [dbo].[ARdataUploadRecord] ([filesize1],[filesize2],[country]"
                ...
                ", '%s', '%s')"%(filesize1,filesize2, country, factory, machine, recipe, process_date, start_time
...
#     :return:
#     '''
#     zf = zipfile.ZipFile(dirfile_path, "a")

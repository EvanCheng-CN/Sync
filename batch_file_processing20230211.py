            ...
            for i in num_list:
                try:
                    if  int(i) > int(pre_list[3]) and int(i) != int(max(num_list)):
                        send_to_score(conn_sql, cur, pre_list, dir, i, ip, logger)
...
def send_to_score(conn_sql, cur, pre_list, dir, num, ip, logger):
    """
